...
//		test();
	...
	static void test() {
		...
		for (int step = 0; step < 10000; step++) {
			...
			int[] x = new int[n];
			int[] y = new int[n];
			KdTreePointQuery2.Point[] points = new KdTreePointQuery2.Point[n];
			...
			for (int i = 0; i < n; i++) {
				...
				points[i] = new KdTreePointQuery2.Point(x[i], y[i]);
			}
			KdTreePointQuery2 tree = new KdTreePointQuery2(points);
